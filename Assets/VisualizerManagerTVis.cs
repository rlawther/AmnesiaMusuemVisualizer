...
public class VisualizerManagerTVis : MonoBehaviour {
	...
	public VisualizationTVis[] visualizations;
	...
	protected void Start() {
		...
		this.visualizations = new VisualizationTVis[datasets.Length];
		for (int i = 0; i < datasets.Length; i++) {
			var dataset = datasets[i];
			dataset.imageDirectory = Path.GetFullPath(dataset.imageDirectory);

			VisualizationTVis v = this.createVisualization(datasets[i]);
			visualizations[i] = v;
		}
	...
	protected VisualizationTVis createVisualization(Dataset dataset) { 
		GameObject go = new GameObject();
		//string folderName = new DirectoryInfo(rootFolder).Name;
		VisualizationTVis v = go.AddComponent<VisualizationTVis>();		
	...
	protected void syncGPSPathways() {
		...
		for (int i = 1; i < datasets.Length; i++) {
			VisualizationTVis v = this.visualizations[i];
